...
class ResDerainModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.model_names = ['ResDerain']
        # define networks; you can use opt.isTrain to specify different behaviors for training and test.
        self.netResDerain = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, 'resnet_9blocks', gpu_ids=self.gpu_ids)
        if self.isTrain:  # only defined during training time
            ...
            self.optimizer = torch.optim.Adam(self.netResDerain.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        self.pred_Bs = self.netResDerain(self.Os)  # generate output image given the input data_A
